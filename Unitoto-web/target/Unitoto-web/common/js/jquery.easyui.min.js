		...
		$(_d9).unbind().bind("mouseover", function(e) {
			...
			if (tt.hasClass("tree-hit")) {
				...
				if (tt.hasClass("tree-checkbox")) {
					...
					_18a(_d9, _dd[0]);
					var _n= _e0(_d9, _dd[0]);
					if(_n.status==false){
						return false;
					}
					_da.onClick.call(_d9, _n);
				}
			}
			e.stopPropagation();
				...
				for (var i = 0; i < _1c7.length; i++) {
					...
					if(item.status==false){
						cc.push("<div id=\"" + item.domId
								+ "\" class=\"tree-node\" style=\"background-color:#dadada;color:black;cursor:not-allowed\">");
					}else{
						cc.push("<div id=\"" + item.domId
								+ "\" class=\"tree-node\">");
					}
	...
	$.fn.tree.defaults = {
		...
		formatter : function(node) {
			if(node.status==false){
				return node.text+'<span class="icon icon-forbid"></span>';
			}
			return node.text;
		},
	...
	$.fn.textbox.methods = {
		...
		setValue : function(jq, _4db) {
			return jq.each(function() {
				...
				if (_4dc != _4db&&!opts.disabled&&(_4dc||_4db!=opts.originalValue)) {
...
(function($) {
	function _503(_504, _505) {
		...
		function cb() {
			...
			if(func_sys_error($.parseJSON(data))){
				opts.success(data);
			}
			setTimeout(function() {
				f.unbind();
...
(function($) {
	...
	function _5e3(_5e4, _5e5, _5e6) {
		...
		if (!opts.noSetRowHeight&&!dc.body1.is(":empty")
